...
public abstract class Player {
    ...
    public MoveTransition makeMove(final Move move) {
        ...
        final Board transitionBord = move.execute();


        final Collection<Move> kingAttacks = Player.calculateAttacksOnTile(
                transitionBord.currentPlayer().getOpponent().getPlayerKing().getPiecePosition(),
                transitionBord.currentPlayer().getLegalMoves());

        if (!kingAttacks.isEmpty()) {
            return new MoveTransition(board, move, MoveStatus.LEAVES_PLAYER_IN_CHECK);
        }

        return new MoveTransition(transitionBord, move, MoveStatus.DONE);
    }
